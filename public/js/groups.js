document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    const groupsModule = document.querySelector('[data-groups-module]');
    const isRealtimeActive = groupsModule?.dataset.realtimeActive === '1';
    const realtimeInterval = parseInt(groupsModule?.dataset.realtimeInterval, 10) || 5;

    let isPolling = false;
    let realtimeTimer = null;

    async function pollGroupsRealtime() {
        if (isPolling) return;
        // Não atualizar enquanto usuário está interagindo com dialogs ou buscando texto
        const hasOpenDialog = document.querySelector('.template-dialog:not([hidden])') || 
                              document.querySelector('.group-dialog-overlay:not([hidden])');
        ...
        try {
            ...
            const feedUrl = new URL(`${getBaseUrl()}/grupos-whatsapp/feed`, window.location.origin);
            if (currentStatus) feedUrl.searchParams.set('status', currentStatus);
            if (currentSearch) feedUrl.searchParams.set('q', currentSearch);
            feedUrl.searchParams.set('_t', Date.now()); // Força requisição nova sem cache
            ...
            if (data.success) {
                ...
                if (data.metrics) {
                    ...
                    const badgeActive = document.getElementById('badge-active');
                    const badgeInactive = document.getElementById('badge-inactive');
                    if (badgeAll) badgeAll.textContent = data.metrics.total ?? '0';
                    if (badgeActive) badgeActive.textContent = data.metrics.active ?? '0';
                    if (badgeInactive) badgeInactive.textContent = data.metrics.inactive ?? '0';
                }

                // Atualizar rodapé com a telemetria do banco e tempo de carregamento
                if (data.footerHtml) {
                ...
                // Atualizar grid de cards se o usuário não estiver com foco no campo de busca
                const isSearchFocused = document.activeElement === searchInput;
                if (!isSearchFocused && groupsContainer && typeof data.htmlCards === 'string') {
                    groupsContainer.innerHTML = data.htmlCards;
                    applyFilters();
                }
            }
        } catch (err) {
            // Falhas transitórias de conexão são ignoradas silenciosamente
        } finally {
            isPolling = false;
        }
    }
    ...
    if (isRealtimeActive && realtimeInterval > 0) {
        realtimeTimer = setInterval(pollGroupsRealtime, realtimeInterval * 1000);
    }
});
